  ...
  static async updateClaimStatus(claimId, status, dealerId, options = {}) {
    try {
      ...
      if (!['approved', 'rejected'].includes(status)) {
        return { success: false, error: 'Status must be either approved or rejected' };
      }
      
      // Verify the claim exists and is for this dealer's product
      const { data: claim, error: claimError } = await supabase
        .from('warranty_claims')
        .select(`
          *,
          product:product_id(dealer_id)
        `)
        .eq('id', claimId)
        .single();
      ...
      if (claim.product.dealer_id !== dealerId) {
        return { success: false, error: 'You are not authorized to update this claim' };
      }
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          dealer_notes: options.notes || null
        })
        .eq('id', claimId);
